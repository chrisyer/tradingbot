...
def load_asset_dataset(symbol: str, csv_path: str, window: int, train_end_date: str) -> tuple[AssetDataset, AssetDataset]:
    df, features, returns = make_features(csv_path, window=window)
    train_end = np.searchsorted(df["time"].to_numpy(), np.datetime64(train_end_date))

    train_ds = AssetDataset(symbol=symbol, features=features[:train_end], returns=returns[:train_end])
    test_ds = AssetDataset(symbol=symbol, features=features[train_end:], returns=returns[train_end:])
